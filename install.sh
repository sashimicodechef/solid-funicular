...
if grep -qs "ubuntu" /etc/os-release; then
	...
	echo "Must be an Ubuntu Instance"
	exit
fi
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	until [[ -z "$port" || "$port" =~ ^[0-9]+$ && "$port" -le 65535 ]]; do
		echo "$port: invalid port. Please enter a valid port number between 1 and 65535."
		read -p "Port [443]: " port
	done
	[[ -z "$port" ]] && port="443"
	echo
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "Finished!"
	echo
	echo "The client configuration is available in:" /home/ubuntu/clients/"$client.ovpn"
 	mv /root/$client.ovpn /home/ubuntu/clients
	...
	case "$option" in
		1)
			...
			new_client
			echo
			echo "$client added. Configuration available in:" home/ubuntu/clients/"$client.ovpn"
   			mv /root/$client.ovpn /home/ubuntu/clients
			exit
		;;
		2)
